...
import { Activity, Briefcase, CheckCircle, Clock, Eye, Filter, Mail, MessageSquare, Plus, Target, Users, X } from 'lucide-react';
...
export default function CampaignDetailsModal({ campaignId, onClose }: CampaignDetailsModalProps) {
  ...
  // Filters inside Assign Leads Modal
  ...
  // Available unique industries from all leads
  ...
  // Filtered leads based on modal industry and status filters
  const filteredModalLeads = useMemo(() => {
    return allLeads.filter(l => {
      const matchIndustry = filterIndustry === 'all' || (l.industry && l.industry.toLowerCase() === filterIndustry.toLowerCase());
      const matchStatus = filterStatus === 'all' || l.status === filterStatus;
      return matchIndustry && matchStatus;
    });
  }, [allLeads, filterIndustry, filterStatus]);

  const handleSelectFiltered = () => {
    const filteredIds = filteredModalLeads.map(l => l._id);
    // Add all filtered IDs to selected
    const merged = Array.from(new Set([...selectedLeadIds, ...filteredIds]));
    setSelectedLeadIds(merged);
    toast.success(`Selected ${filteredIds.length} lead(s)`);
  };

  const handleDeselectFiltered = () => {
    const filteredIdsSet = new Set(filteredModalLeads.map(l => l._id));
    setSelectedLeadIds(selectedLeadIds.filter(id => !filteredIdsSet.has(id)));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
      ...
      {/* Add Leads Picker Modal with Industry and Status Filtering */}
      {showAssignModal && (
          ...
          <div className="w-full max-w-2xl rounded-xl border border-border bg-card p-6 shadow-2xl flex flex-col max-h-[85vh]">
            <div className="flex items-center justify-between pb-4 border-b border-border">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Filter by industry or status, select leads, and assign them in bulk.</p>
              </div>
              <button onClick={() => setShowAssignModal(false)} className="rounded-full p-1 hover:bg-muted text-muted-foreground hover:text-foreground">
                <X className="h-5 w-5" />
              </button>
            </div>

            {/* Filter Bar */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 py-3 border-b border-border bg-muted/20 px-3 rounded-lg my-3">
              {/* Filter by Industry */}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-muted-foreground flex items-center gap-1.5">
                  <Briefcase className="h-3.5 w-3.5" /> Filter by Industry:
                </label>
                <select
                  value={filterIndustry}
                  onChange={(e) => setFilterIndustry(e.target.value)}
                  className="w-full h-8 text-xs rounded-md border border-input bg-background px-2 font-medium outline-none focus:ring-1 focus:ring-primary"
                >
                  <option value="all">All Industries ({allLeads.length})</option>
                  {availableIndustries.map(ind => (
                    <option key={ind} value={ind}>
                      {ind} ({allLeads.filter(l => l.industry?.toLowerCase() === ind.toLowerCase()).length})
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="flex items-center justify-between py-2 text-xs text-muted-foreground">
              <span>Showing <strong>{filteredModalLeads.length}</strong> matching leads</span>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={handleSelectFiltered}
                  className="px-2.5 py-1 rounded bg-primary/10 text-primary hover:bg-primary/20 font-medium transition-colors"
                >
                  Select All Filtered ({filteredModalLeads.length})
                </button>
                <button
                  type="button"
                  onClick={handleDeselectFiltered}
                  className="px-2.5 py-1 rounded bg-muted hover:bg-muted/80 font-medium transition-colors"
                >
                  Deselect Filtered
                </button>
              </div>
            ...
            {/* Leads List */}
            <div className="flex-1 overflow-y-auto my-2 space-y-2 border border-border rounded-lg p-2 bg-background/50">
              {filteredModalLeads.length === 0 ? (
                <div className="p-8 text-center text-xs text-muted-foreground">
                  No leads found matching the selected Industry or Status filter.
                </div>
              ) : (
                filteredModalLeads.map((l: any) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-lg border text-sm cursor-pointer transition-colors ${
                        isChecked ? 'bg-primary/10 border-primary/40' : 'bg-card border-border hover:bg-muted/50'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <input
            ...
            <div className="flex justify-between items-center pt-4 border-t border-border mt-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 disabled:opacity-50 transition-colors shadow-sm"
                >
                  {assigning ? 'Assigning...' : `Assign ${selectedLeadIds.length} Lead(s) to Campaign`}
                </button>
              </div>
            </div>
          </div>
